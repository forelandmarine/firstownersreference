...
export async function generateMetadata(props: {
  ...
  const spread = getDataSpread(slug);
  if (!section || !spread) return {};
  return {
    title: `${spread.title} | Section ${section.number} data spread`,
    description: spread.standfirst,
  };
}

export default async function DataSpreadPage(props: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await props.params;
  const section = getSection(slug);
  const spread = getDataSpread(slug);
  if (!section || !spread) notFound();
          ...
          <div className="lg:col-span-8 lg:col-start-3 space-y-12">
            {spread.blocks.map((block, i) => {
              ...
              if (block.type === "paragraph") {
                return (
                  <p key={i} className="font-serif text-lg leading-relaxed text-charcoal-soft max-w-prose">
                    {block.text}
                  </p>
                );
              }
                      ...
                      {block.rows.map((row, ri) => (
                        ...
                        >
                          ...
                          <dd className="md:col-span-7">
                            ...
                            {row.note && (
                              <p className="meta mt-1">{row.note}</p>
                            )}
                          </dd>
                        </div>
                      ))}
